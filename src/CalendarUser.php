...
class CalendarUser extends Component
{
    ...
    public function mount(){
       ...
       $id_event=day::where('timestamp',$today['timestamp'])->pluck('id')[0];
        $this->events=day::find($id_event)->events()->get();
    ...
    public function show_events($timestamp){
        $id_event=day::where('timestamp',$timestamp)->pluck('id')->first();
        $this->events=day::find($id_event)->events()->get();
        $this->render();
    }
    ...
    public function render()
    {
        ...
        return view('livewire.calendar-user',['Calendar_Alignment'=>$Calendar_Alignment,'month_of_year'=>$month_of_year,'year'=>$this->year])->layout('layouts.app');
    }
}
